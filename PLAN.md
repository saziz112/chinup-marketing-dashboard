...
15. ✅ **Ads** — verified 2026-07-15:
    ...
    - Stray caller to deleted `/api/content/publish-due`: ✅ RESOLVED 2026-07-16 — n8n workflow "Publish Scheduled Posts" (h6Pr2ddwA1ANDo8D) deactivated via n8n API (reached over HTTP; the earlier NO_RESPONSE was port 443 being down on the n8n VPS, port 80 works).
...
### Pre-work findings (2026-07-15, read-only recon while Sam away — NOTHING sent, NOTHING shipped)
Scoped items 20 + 21 against the live DB. Neither was built: 19/20 send patient texts (need Sam's copy + dry-run per the campaign-criteria rule); 21's core metric proved unreliable (below). All sends are **manual** — the only crons are analyze-conversations / sync-zenoti / sync-research, none send campaigns — so a new segment is inert until someone clicks send.
- **Item 20 feasibility: GOOD.** No-show/cancellation data now lands post-item-14 fix: last 30d = **29 NoShow** (past missed appts, dates 6/29–7/15) + **99 Cancelled** (7/1–7/30, incl. future). Sources: Zenoti Cancelled 99 / NoShow 1, MindBody NoShow 28 (historical). Ready to build once Sam sets: lookback window (e.g. no-show in last 7–14d), suppress-if-rebooked (reuse the peer-expansion from item 16), cooldown, copy/tone (follow softened-maintenance precedent + DELETE opt-out + per-location phone), channel.
- **Item 21 BLOCKER: holdout-lift metric is not reliably computable with current instrumentation.** Tried 3 methodologies to reproduce item 18's maintenance 16.0% treated / 14.8% holdout and got 9.4/1.9 (per-row, sent_at anchor), then 13.6/17.1 (distinct-patient, run_at anchor) — none match, and the sign flips. Root causes in the send route (`ghl-reactivation` POST ~line 780): (a) controls re-logged **every run** → 1,851 holdout rows vs ~187 distinct patients (dup inflation); (b) holdout rows get **no `sent_at`** (only `run_at` via the run is a valid anchor); (c) the deterministic ~12% holdout set drifts as the due-population changes. **Recommendation:** before any scoreboard trusts lift, redesign the holdout instrumentation (log each control once with a fixed anchor, or compute lift from distinct-patient first-exposure) — a measurement decision for Sam. The *non-lift* parts (per-segment runs/targeted/sent + single-arm "booked within 30d of outreach" + revenue) ARE computable and honest if labeled non-causal.

19. ⬜ **Maintenance split by last treatment** — segment cadence + messaging by last treatment received (Botox/filler/Sculptra/laser/etc.); verify `normalizeTreatment()` handles Zenoti item names. (Modifies the LIVE maintenance send — dry-run + diff before ship.)
20. 🔄 **No-show recovery campaign** — BUILT 2026-07-15 (commit 64f5a20), inert until manually run. Segment = NoShow **or** Cancelled (Sam's call — Zenoti barely uses NoShow, ~1/wk; volume is in Cancelled), past-dated, last 14d, with the maintenance cross-source suppression (already-rebooked + seen-since, peer-expanded). New `getNoShowRecoveryPatients()` + GET branch + UI picker + 3 SMS variants + email (gentle judgment-free win-back, DELETE opt-out, per-location phone). Dry-run: **48 eligible, all GHL-matched, all 3 locations** (Decatur 17 / Kennesaw 17 / Smyrna 14) before cooldown/DND.
    - **Copy signed off 2026-08-09 with two changes (Sam's call):** (a) **3-day minimum age** added (`minAgeDays`, new 3rd arg to `getNoShowRecoveryPatients`) — the 0–14d window would text people the morning after they deliberately cancelled, which reads as surveillance; (b) **copy softened + emoji dropped from SMS** — no variant now says the visit "didn't happen"/"didn't work out" (96% of the segment is *Cancelled*, i.e. patients who told us), and removing ☺️ moves SMS from UCS-2 to GSM-7, **4–5 segments → 2**. Emails keep ☺️ (no segment cost), same softened wording.
    - Live dry-run 2026-08-09 after changes: **51 eligible at 0-day floor → 38 at 3-day floor**, all 38 GHL-matched (Decatur 16 / Smyrna 14 / Kennesaw 8; 36 Cancelled / 2 NoShow), before cooldown/DND.
    - **Known UI bug (not fixed):** `buildForecast` bills SMS at a flat $0.02/message, ignoring multi-segment encoding — it understated the old copy's cost ~4–5×, ~2× even now. Cosmetic at this volume ($1.52 actual vs $0.76 shown for 38 sends).
    - **Pending: first manual send.**
21. ⬜ **Campaign scoreboard** — per-campaign funnel: sent → replied → booked → showed → revenue, with holdout lift where instrumented. **BLOCKED on lift-instrumentation redesign (see pre-work findings); non-lift funnel is buildable now if wanted.**
